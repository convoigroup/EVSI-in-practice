prac.working.directory <- file.path("C:/Users/anna heath/Google Drive/PhD Files/EVSI/Practical Paper")
setwd(prac.working.directory)

set.seed(1234) # It is a good practice to set the seed so results can be replicated

# The following R packages are needed.  
lib.loc <- NULL                         # Set the path to were you have installed packages, if different from the default location
library(BCEA, lib.loc = lib.loc)        # Bayesian Cost-Effectiveness Analysis
library(R2OpenBUGS, lib.loc = lib.loc)  # Interfaces R with OpenBUGS
...
library(BCEA)
prior<-bcea(e,c,ref=2)
plot(prior)

###EVPPI
wtp<-30000
#evppi.half<-evppi(c("pi1","rho","gamma","gamma2","lambda.2.3.fix",
#                    "lambda.1.3.fix","SE[1]","SE[2]"),
#                  prior.model$BUGSoutput$sims.matrix,prior)
#print(evppi.half$evppi[which(prior$k==wtp)])
